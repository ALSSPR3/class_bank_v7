...
public class AccountController {
	...
	public String savePage() {
		...
		if (principal == null) {
			throw new UnAuthorizedExeception(Define.NOT_AN_AUTHENTICATED_USER, HttpStatus.UNAUTHORIZED);
		}
		return "account/save";
	}
	...
	public String withdrawalProc(WithdrawalDTO dto) {
		...
		if (dto.getAmount() == null) {
			throw new DataDeliveryException(Define.ENTER_YOUR_BALANCE, HttpStatus.BAD_REQUEST);
		} else if (dto.getAmount().longValue() <= 0) {
	...
	/**
	 * 계좌 상세보기 페이지
	 * 주소 설계 : localhost:8080/account/detail/${1}?type=all, deposit, /account/detail/1?type=withdrawal
	 * @return
	 */
	...
	public String detail(@PathVariable(name = "accountId") Integer accountId, @RequestParam(required = false, name = "type") String type, Model model) {
		...
		List<HistoryAccount> historyList = accountService.readHistoryByAccount(type, accountId);
